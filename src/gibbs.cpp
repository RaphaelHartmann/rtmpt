...
namespace drtmpt {
  ...
  void gibbs_times_new(const std::vector<trial> & daten,  std::vector<gsl_rng*>& rsts) {
        ...
        for (int j = 0; j != sample_size2 / NOTHREADS; j++) {
          for (int i = 0; i != n_all_parameters; i++) {
            ...
            if (raustemp >> x) {
              ...
              error("Failed to read double from input stream (raustemp) in gibbs_times_new.");
            }
          }
          rausFiles << std::endl;
        }
    ...
    if (((phase == 1) && (offset + ireps >= PHASE1)) && (rmax <= 50.0)) {
      supersig.assign(static_cast<size_t>(NOTHREADS * n_all_parameters * n_all_parameters), 0.0);
      phase = 2;
      goto RESTART;
    }
  ...
  #define COMPLETE_SAMPLE(S,I,P) complete_sample[(S)*(sample_size/NOTHREADS)*(n_all_parameters) + (I)*(n_all_parameters)+P]
    ...
    if (goon) {
      ...
      for (int is = 0; is != NOTHREADS; is++) {
        rausFiles.open(std::string(TMPDIR) + "temp" + std::to_string(is), std::ios_base::app);
        rausFiles << std::setprecision(12);
        ...
        rausFiles.close();
        rausFiles.clear();
        std::string t = std::string(TMPDIR) + "raus" + std::to_string(is);
        std::remove(t.c_str());
        std::string s = std::string(TMPDIR) + "temp" + std::to_string(is);
        std::rename(s.c_str(), t.c_str());
      }
      std::string tempPath = std::string(TMPDIR) + "temp";
