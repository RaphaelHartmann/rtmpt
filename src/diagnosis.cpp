...
// namespace rtsNS {
	...
	void quantiles(vector<trial> daten, int n_all_parameters, double *sample) {
		...
		Rprintf("delta in ms per group [median, 95 and 99%% HDI]\n");
	...
	//			 pij[k]=1.0;
			for (int ir = 0; ir != NDRIN(j, k); ir++) {
				...
				d0ha = (ix > 0) ? X_FOR_ALL(t, ip) : (1.0 - X_FOR_ALL(t, ip));
				pij[k] = pij[k] * d0ha;
			}
			pj += pij[k];
		}
	...
	void make_tij_for_one_trial_new(trial one, double *rhos, double *lambdas, double *restpars, double *pij) {
		...
		for (int k = 0; k != branch[j]; k++) {
			...
			if (complength == 0) { pij[k] = exp(-0.5*gsl_pow_2((rt - rmu) / rsig)) / sqr2pi / rsig; }
			if (complength == 1) {
				double lam = lams[0]; pij[k] = exp(logexgaussian(lam, rmu, rsig, rt));
			...
			if (complength >= 2) {
				...
				pij[k] = elogdiff(hypo_plus, hypo_minus);
				if (pij[k] < 0.0) {
	...
	void dic(int N_all_parameters, vector <trial> daten, double *beta, double *sample) {
		...
		for (int is = 0; is != SAMPLE_SIZE; is++) {
			...
			for (int x = 0; x != trialno; x++) {
				...
				double xsi = gsl_cdf_ugaussian_P(rmu / rsig);
				...
				if (log_lik_flag) log_lik << setw(20) << log(p);
				persample += -2 * log(p);
			}
		...
		for (int x = 0; x != trialno; x++) {
			...
			make_pij_for_one_trial_new(one, x_for_all, pij, p);
			p /= xsi;
			if (p <= 0) Rprintf("DIC loglik Problem in pd\n");
			pd += -2 * log(p);
		}
