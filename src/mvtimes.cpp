...
// namespace rtsNS {
	...
	void make_tij_for_one_trial_new(trial one, double *rhos, double *lambdas, double *xlams, double *restpars, double* slams, double *pij) {
		...
		for (int k = 0; k != branch[j]; k++) {
			...
			if (complength == 1) {
				...
				if (DEBUG) {if (exp(pij[k]) < 0.0) Rprintf("pij[k] < 0; pf = 1 %g\n", pij[k]);} //std::cout << "pij[k] < 0; pf =1" << pij[k] << std::endl;
			}
			if (complength >= 2) {
				...
				if (dif < 0) 	pij[k] = hypo_plus + gsl_log1p(-exp(hypo_minus - hypo_plus)); else pij[k] = -sqrt(DBL_MAX);
	...
	#define PARMON(I,J) parmon[(I-1)*n_all_parameters+J]
		...
		Rprintf("R-statistic mean THETAS\n"); //std::cout << "R-statistic MUS" << std::endl;
		int jz = -1;
		for (int ig = 0; ig != igroup; ig++) {
		...
		Rprintf("R-statistic mean LAMBDAS_MINUS\n"); //std::cout << "R-statistic RHOS MINUS" << std::endl;
		jz = ifree * igroup;
		for (int ig = 0; ig != igroup; ig++) {
		...
		Rprintf("mean LAMBDAS_MINUS\n"); //std::cout << "RHOS MINUS" << std::endl;
		jz = ifree * igroup;
		for (int ig = 0; ig != igroup; ig++) {
		...
		Rprintf("R-statistic mean LAMBDAS_PLUS\n"); //std::cout << "R-statistic RHOS PLUS" << std::endl;
		int js = jz;
		for (int ig = 0; ig != igroup; ig++) {
		...
		Rprintf("mean LAMBDAS_PLUS\n"); //std::cout << "RHOS PLUS" << std::endl;
		jz = js;
		for (int ig = 0; ig != igroup; ig++) {
		...
		Rprintf("R-statistic mean DELTAS, OMEGA^2\n"); //std::cout << "R-statistic RESTPARS" << std::endl;
		...
		Rprintf("\n"); //std::cout << std::endl;
		Rprintf("mean DELTAS, OMEGA^2\n"); //std::cout << "RESTPARS" << std::endl;
		...
		Rprintf("Iterations %d\n", (irun + 1)*ireps); //std::cout << "Iterationen " << (irun + 1)*ireps << std::endl << std::endl;
		if (kernpar > igroup*respno) {
			for (int i = 0; i < kernpar; i++) Rprintf("---------------");
		} else {
			for (int i = 0; i < (igroup*respno); i++) Rprintf("---------------"); Rprintf("---------------");
		}
		Rprintf("\n\n");
		if (sig) free(sig);
	} // end on_screen3
	...
	void belege_bridge(int ithread, int ix, int n_bridge_store, double *bridge_sample, double *mu, double *lams,
		...
		gsl_linalg_cholesky_decomp1(cy);
		jj = 0;
