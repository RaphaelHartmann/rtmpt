...
namespace ertmpt {
  ...
  //		for (int i = 0; i != a; i++) std::cout << std::setw(5) << r[i] << std::setw(12) << lams[i] << std::setw(12) << mu << std::setw(12) << sd << std::endl;
  ...
  */
  ...
  bool trouble_shooter(int &a, std::vector<int> &r, double* lams, double* loglams) {
  ...
  double logf_tij(int a, std::vector<int> r, double *lams, double *loglams, double mu, double sd, double t) {
  	...
  	for (int i = 0; i != a; i++) b += log(lams[i])* r[i];
  	NEW: double temp = 0.0;
  	std::vector<double> hypoplus, hypominus;
  	...
  	if (hplus <= hminus) {
  		...
  		// printf("%g %g %g\n", hplus, hminus, t);
  		//		char x; std::cin >> x;
  		//		for (int i = 0; i != hypoplus.size(); i++) std::cout << std::setw(20) << hypoplus[i]; std::cout << std::endl;
  		//		for (int i = 0; i != hypominus.size(); i++) std::cout << std::setw(20) << (hypominus[i]); std::cout << std::endl;
  		...
  		// 	std::cout << "hallo" << std::setw(20) << temp << std::endl;
  		// }
  	}
